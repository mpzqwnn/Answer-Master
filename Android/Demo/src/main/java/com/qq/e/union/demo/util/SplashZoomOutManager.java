...
public class SplashZoomOutManager {
  ...
  private final int zoomOutWidth;//悬浮窗的宽度
  private final int zoomOutHeight;//悬浮窗的高度
  private final int zoomOutMargin;//悬浮窗最小离屏幕边缘的距离
  private final int zoomOutAbove;//悬浮窗默认距离屏幕底端的高度
  private final int zoomOutPos;//悬浮窗默认位于屏幕左面或右面
  private final int zoomOutAnimationTime;//悬浮窗缩放动画的，单位ms
  ...
  private final int[] originSplashPos = new int[2];
  ...
  private SplashZoomOutManager() {
    ...
    zoomOutHeight = Math.round((float) (zoomOutWidth * 16) / 9);//根据宽度计算高度，之前使用PxUtils.dpToPx(context, 160);
